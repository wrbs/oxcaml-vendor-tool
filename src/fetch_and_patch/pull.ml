...
let write_dune ~dest_dir ~packages =
  let deps =
    Set.to_list packages
    |> List.map ~f:(fun name -> [%sexp [ "package"; (name : Opam.Package.Name.t) ]])
  in
  let alias =
    [%sexp [ "alias"; [ "name"; "vendored-pkgs" ]; "deps" :: (deps : Sexp.t list) ]]
  in
  let%bind contents =
    Process.run_exn
      ~prog:"dune"
      ...
      ()
  in
  Writer.save (dest_dir ^/ "dune") ~contents
;;
  ...
  fun () ->
    ...
    | Ok () ->
      ...
      let packages =
        Map.data dirs
        |> List.map ~f:(fun dir ->
          Set.map (module Opam.Package.Name) dir.provides ~f:OpamPackage.name)
        |> Opam.Package.Name.Set.union_list
      in
      write_dune ~dest_dir ~packages
;;
